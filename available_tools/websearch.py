...
def web_search_func(query: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if urls_to_extract:
            ...
            for url in urls_to_extract:
                try:
                    ...
                    logger.warning(f"Failed to extract content from {url}: {extract_error}")
        ...
        for i, result in enumerate(search_results):
            ...
            for extracted in extracted_content:
                if extracted["url"] == result.get("url"):
                    enhanced_result["extracted_content"] = extracted["extracted_content"]
        ...
        logger.info(f"✅ Search completed: {len(enhanced_results)} results, {len(extracted_content)} with extracted content")
