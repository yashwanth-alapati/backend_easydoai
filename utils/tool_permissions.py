...
def requires_approval(tool_name: str, subtool: Optional[str] = None) -> bool:
    ...
    if isinstance(TOOL_PERMISSIONS[tool_name], dict):
        if subtool is None:
            return True  # Require approval if subtool not specified for tool with subtools
        return TOOL_PERMISSIONS[tool_name].get(subtool, ToolPermission.REQUIRE_APPROVAL) == ToolPermission.REQUIRE_APPROVAL
...
def format_approval_request(tool_name: str, tool_args: Dict[str, Any]) -> str:
    ...
    if tool_name == "gmail_mcp":
        ...
        if subtool == "send_email":
            return (f"Request to send email:\n"
                   f"To: {args.get('to', [])}\n"
            ...
            return (f"Request to modify email(s):\n"
