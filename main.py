...
from mongodb_config import get_mongodb_client, close_mongodb_connection, is_mongodb_available
...
async def add_message(task_id: str, req: Request, db: Session = Depends(get_db)):
    ...
    agent = EasydoAgent()
    reply = await agent.process_message(user_message, conversation_history=conversation_history)
...
async def chat_endpoint(req: ChatRequest, db: Session = Depends(get_db)):
    ...
    chat_service = ChatService()
    context_messages = get_last_n_chat_messages_from_mongodb(chat_service, user.id, n=10)
...
def get_last_n_chat_messages_from_mongodb(chat_service: ChatService, user_id: int, n: int = 10):
