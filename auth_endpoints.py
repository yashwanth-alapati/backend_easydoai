...
async def google_oauth_callback(
    ...
    try:
        ...
        success = token_storage.store_tokens(
            result["user_id"], result["service"], result["tokens"]
        )

        if not success:
            raise HTTPException(
                status_code=500, detail="Failed to store authentication tokens"
            )

        logger.info(
            f"Successfully stored tokens for user {result['user_id']}, service {result['service']}"
        )
...
async def check_authorization(
    ...
    if tokens:
        ...
        return AuthStatusResponse(authorized=False, service=service, user_id=user_id)
...
async def list_user_services(user_id: str):
    ...
    return {"user_id": user_id, "authorized_services": services}
...
async def revoke_authorization(
    ...
    if tokens:
        ...
        raise HTTPException(
            status_code=404, detail=f"No authorization found for {service}"
        )
